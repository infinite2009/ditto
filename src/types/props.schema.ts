...
export interface TemplateKeyPathsReg {
  ...
  columnKey?: string;
  // 自定义生成代码（传入该字段会将自定义代码注入dsl，并在生成代码处直接使用dsl生成指定字段的函数片段）
  customGenerateCode?: string[];
 ...
 */
export default interface IPropsSchema<Value = any> extends ISchema {
  ...
  title: string;
  /**
   * 配置的 props 值，同一个组件的不同配置下，这个 value 是不一样的。但是它们的 defaultValue 是一样的
   * 如果存在 keyPath，里边的对应值会变成 template 的引用
   * 如果 valueSource 的值为 handler, 则 value 的值是 { action: string; eventTrackingInfo: string }
   * 如果 valueSource 的值为 variable，value 的值是状态变量名
   */
  value?: Value;
  ...
  valueType:
    ...
    | 'dayjs'
    | 'array'
    | Array<'string' | 'boolean' | 'number' | 'function' | 'object' | 'array' | 'dayjs'>;
}
